...
const RecentlyViewed = ({
    ...
    const filteredRecentlyRecipes = recentlyRecipes.filter(
    ...
    useEffect(() => {
        ...
        if (!recentlyViewedIds) {
            ...
            const parsedRecentlyViewedIds: string[] =
                JSON.parse(recentlyViewedIds);
            if (parsedRecentlyViewedIds.every((id) => id !== currentId)) {
                parsedRecentlyViewedIds.unshift(currentId.toString());
                const trimmedIds = parsedRecentlyViewedIds.filter(
                ...
                parsedRecentlyViewedIds
                    .splice(
                        parsedRecentlyViewedIds.indexOf(currentId.toString()),
                        1
                    )
    ...
    return (
        <section className="recently-views">
            {filteredRecentlyRecipes.length > 0 && (
                <>
                    <h2 className="recently-views__title">
                        Нещодавно переглянуті
                    </h2>
                    <RecipesSlider
                        slides={filteredRecentlyRecipes}
                        options={OPTIONS}
                    />
                </>
            )}
