...
export const manageFavoritesRecipes = createAsyncThunk(
    ...
    async function (
        { recepieId, uid }: { recepieId: string | number | null; uid: string },
        { rejectWithValue }
    ) {
        try {
            const db = getDatabase();
            ...
            if (
                typeof recepieId === 'string' ||
                typeof recepieId === 'number'
            ) {
...
export const favoriteRecipesSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(
            fetchFavoritesRecipe.fulfilled,
