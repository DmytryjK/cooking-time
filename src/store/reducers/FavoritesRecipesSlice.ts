...
import { filterByIngredients, uniqSearchedTags, filterByName } from "../../helpers/filterRecipes";
...
export const manageFavoritesRecipes = createAsyncThunk(
  ...
  async function ({ item, uid }: { item: Recipe; uid: string }, { rejectWithValue }) {
    try {
      const { id } = item;
      const db = getDatabase();
...
export const favoriteRecipesSlice = createSlice({
  ...
  reducers: {
    ...
    filterFavoriteRecipes: (state, action: PayloadAction<PayloadActionFilter>) => {
      const { searchInput, searchTags, searchCategories } = action.payload;
      state.filteredFavoriteRecipes = JSON.parse(JSON.stringify(state.favoriteRecipes));
      ...
      if (searchInput && searchTags.length > 0 && searchCategories.length > 0) {
        state.filteredFavoriteRecipes = state.filteredFavoriteRecipes
          .filter((recipe) => {
            const { isCorrectValue } = filterByName(recipe, searchInput);
            return isCorrectValue;
          })
          .filter((recipe) => {
            const { isCorrect, searchedConstruction } = filterByIngredients(recipe, searchTags);
            if (isCorrect && searchedConstruction.recipeIngredient !== "") {
              searchedWord.push({
        ...
        state.filteredFavoriteRecipes = state.filteredFavoriteRecipes.filter((recipe) => {
          const { isCorrectValue } = filterByName(recipe, searchInput);
          return isCorrectValue;
        });
        ...
        state.filteredFavoriteRecipes = state.filteredFavoriteRecipes.filter(
          (recipe) => searchCategories.includes(recipe.category) === true,
        );
      }
      if (searchedWord.length > 0 && searchTags.length > 0) {
        state.searchedTagFilledFavorites = [...state.searchedTagFilledFavorites, ...uniqSearchedTags(searchedWord)];
      }
    },
    setCurrentFilteredFavoriteRecipes: (state, action: PayloadAction<Recipe[]>) => {
      state.filteredFavoriteRecipes = action.payload;
    },
    localRemoveFavoriteRecipe: (state, action: PayloadAction<string | number>) => {
      state.favoriteRecipes = state.favoriteRecipes.filter((recipe) => recipe.id !== action.payload);
      state.filteredFavoriteRecipes = state.filteredFavoriteRecipes.filter((recipe) => recipe.id !== action.payload);
    },
    removeSearchedTagFilledFavorites: (state, action: PayloadAction<number>) => {
      state.searchedTagFilledFavorites = state.searchedTagFilledFavorites.filter(
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchFavoritesRecipes.fulfilled, (state, action: PayloadAction<Recipe[]>) => {
      state.favoriteRecipes = action.payload;
      state.loadingRecipesById = "succeeded";
    });
    builder.addCase(fetchFavoritesRecipes.rejected, (state, action: PayloadAction<unknown>) => {
    ...
    builder.addCase(manageFavoritesRecipes.fulfilled, (state, action: PayloadAction<Recipe | undefined>) => {
      if (!action.payload) return;
      const { id, favorites } = action.payload;
      if (state.favoriteRecipes.some((favoriteItem) => favoriteItem.id === id)) {
        state.favoriteRecipes = state.favoriteRecipes.filter((item) => item.id !== id);
        state.filteredFavoriteRecipes = state.filteredFavoriteRecipes.filter((item) => item.id !== id);
      } else {
        state.favoriteRecipes = [...state.favoriteRecipes, { ...action.payload, favorites: true }];
        state.filteredFavoriteRecipes = [...state.filteredFavoriteRecipes, { ...action.payload, favorites: true }];
      }
