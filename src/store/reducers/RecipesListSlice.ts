...
import { filterByIngredients, filterByName, uniqSearchedTags } from "../../helpers/filterRecipes";
        ...
        get(itemRef)
          .then((snapshot) => {
            ...
            if (snapshot.val().imgDto.length > 0) {
              snapshot.val().imgDto.forEach((item: { id: string; src: string }) => {
                const storageReference = storageRef(storage, item.src);
                deleteObject(storageReference).catch((e) => {
                  alert("Щось пішло не так, спробуйте ще раз");
                });
              });
            }
            ...
            get(favoritesRef).then((snapshot) => {
              if (snapshot.exists()) {
                snapshot.forEach((childSnapshot) => {
                  ...
                  if (userData && userData.some((data: string) => data === id) && userId) {
                    const userDishesRef = ref(db, `/favorites/${userId}/${userData.indexOf(id)}`);
...
export const recepieListSlice = createSlice({
  ...
  reducers: {
    ...
    filterRecipes: (state, action: PayloadAction<PayloadActionFilter>) => {
      ...
      if (searchInput && searchTags.length > 0 && searchCategories.length > 0) {
        ...
        state.filteredRecipes = state.filteredRecipes.filter((recipe) => {
          const { isCorrectValue } = filterByName(recipe, searchInput);
          return isCorrectValue;
        });
      ...
      if (searchedWord.length > 0 && searchTags.length > 0) {
        state.searchedTagFilled = [...state.searchedTagFilled, ...uniqSearchedTags(searchedWord)];
      }
    },
    resetRecipes: (state) => {
      state.recipes = [];
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(updateRecipe.rejected, (state, action: PayloadAction<unknown>) => {
      state.loadingForm = "failed";
      state.error = action.payload;
    });
    ...
    builder.addCase(deleteRecipe.rejected, (state, action: PayloadAction<unknown>) => {
