import { useState, useEffect, SetStateAction, Dispatch, useContext, useRef, KeyboardEvent } from "react";
...
const SelectUnitsItem = ({
  ...
  const closeSelect = (e: any) => {
    if (!e.target.closest(".sort-unit__custom-fields") && !e.target.closest(".sort-unit")) {
  ...
  useEffect(() => {
    ...
    if (selectRef.current && window.visualViewport && fieldsetRef.current) {
      const bottomDistance = window.visualViewport.height - selectRef.current.getBoundingClientRect().bottom;
      const fieldsetHeight = fieldsetRef.current.getBoundingClientRect().height;
      fieldsetRef.current.style.setProperty("--unitSelectHeight", "auto");
      if (bottomDistance > 0 && fieldsetHeight > bottomDistance) {
        fieldsetRef.current.style.setProperty("--unitSelectHeight", `${Math.floor(bottomDistance) - 20}px`);
      }
    }
  ...
  useEffect(() => {
    if (searchUnitValue) {
      const filteredUnits = units.filter((unit) => unit.startsWith(searchUnitValue));
  ...
  const keydownInput = (e: KeyboardEvent<HTMLInputElement>) => {
    if ((e.key === "Enter" || e.code === "Enter") && filteredUnits.length > 0) {
      setCurrentUnit(isUnitFocused || filteredUnits[0]);
      setIsUnitsOpen(false);
    }

    if ((e.key === "ArrowDown" || e.code === "ArrowDown") && filteredUnits.length > 0) {
      setIsUnitFocused((prev) => {
        if (!filteredUnits[filteredUnits.indexOf(prev) + 1]) return prev;
        return filteredUnits[filteredUnits.indexOf(prev) + 1];
      });
    }

    if ((e.key === "ArrowUp" || e.code === "ArrowUp") && filteredUnits.length > 0) {
      setIsUnitFocused((prev) => {
        if (!filteredUnits[filteredUnits.indexOf(prev) - 1]) return prev;
        return filteredUnits[filteredUnits.indexOf(prev) - 1];
      });
  ...
  return (
    <div className={`sort-unit ${isUnitsOpen ? "active" : ""}`} id={`${tagId}`}>
      <button
      ...
      >
        ...
        <input
          className={`sort-unit__search-input ${isUnitsOpen ? "active" : ""}`}
          type="text"
