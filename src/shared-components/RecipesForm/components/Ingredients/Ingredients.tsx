import { useState, useEffect, createContext, Dispatch, SetStateAction, useMemo, useCallback, memo } from "react";
...
const Ingredients = ({ localingredients }: { localingredients: IngredientsType[] | undefined }) => {
  ...
  const contextValue = useMemo(() => ({ selectedUnits, setSelectedUnits }), [selectedUnits, setSelectedUnits]);
  ...
  const [dragIngredients, setDragIngredients] = useState<IngredientsType[] | undefined>(localingredients);
  ...
  return (
    <fieldset className="tagsForm">
      ...
      <div className="tagsForm__top-wrapper">
        ...
        />
        <button className="tagsForm__search-btn" onClick={handlePlusBtn} type="button" aria-label="додати інгредієнт" />
      </div>
      {ingredients.length === 0 ? (
        ...
        <LazyMotion features={domAnimation}>
          ...
          >
            {(isIngredientDragEnd.isDragEnd ? ingredients : dragIngredients || ingredients).map((ingredient) => {
              const id = `${ingredient.id}`;
              return (
                <SelectUnitContext.Provider key={id} value={contextValue}>
                  <Ingredient
                    key={id}
                    ingredient={ingredient}
                    setIsIngredientDragEnd={setIsIngredientDragEnd}
                    isIngredientDragEnd={isIngredientDragEnd}
                  />
        ...
        <div className="duplicated-block">
          ...
          <ul className="duplicated-list">
            ...
            {duplicatedValues.map((item, index) => {
              return (
                <li className="duplicated-list__item" key={nextId(item)}>
                  {item}
                </li>
              );
            })}
